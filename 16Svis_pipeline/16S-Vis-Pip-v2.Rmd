---
title: "Standard_visualization_v1"
...
x<-c("tidyverse", "ggplot2", "reshape2","broom", "dplyr", "data.table","ape", "docopt",
...
--wdir PATH         give path to working dir, [e.g.: '~/Documents/CoreMicrobiome/Julianas_V4_OTUtab/date_072222'] 
--map FILENAME      give name to mapping file [def: mapping.txt]
--outdir DIRNAME    give name to output directory [def: 16Svis_plots]
--title NAME        give name for title of report
--sampDepth NUM     choose a sampleReadDepth for filtering [def:10000]
--asvDepth NUMB      chose an asvDepth for filtering [def: 10]"
doc
```
...
output$path=(paste0(input$wdir ,output$name)); dir.create(output$path)
...
data$raw$amp = amp_load(cbind(data$raw$counts, data$raw$tax), data$meta)
data$raw$amp
...
```
A sampleDepth of 6000 was chosen as most rarecurves already reached plateau by this value
...
data$flt$amp=amp_subset_samples(data$exp$amp, minreads=readDepth)
rem=ncol(data$exp$amp$metadata)-ncol(data$flt$amp$abund)
...
# cat("------- removing ASVs represented by prevalence in <", asvPrev, "samples")
    ...
    # data$flt$counts = data$exp$amp$abund[ prevdf > prevth , ];                              # filtering based on prev%
    # dim(data$flt$counts)

# filtering raw-ASVs, using prev#
# prevdf =  apply(data$exp$amp$abund, MARGIN=1, FUN = function(x){sum(x>0)}) # calc prev# for each ASV
# data$flt$counts = data$exp$amp$abund[ prevdf >= asvPrev , ];  #(1)          # filter based on prev#
#   # dim(data$flt$counts)

#filtering flt-ASVs, by asvDepth as well
  # data$flt$counts = data$flt$counts[ rowSums(data$flt$counts) >= asvDepth , ] # filter based on asvDepth
...
# OR filtering raw-ASVs by ONLY asvDepth
    ...
    data$flt$counts = data$exp$amp$abund[ rowSums(data$exp$amp$abund) >= asvDepth , ] # filter based on asvDepth
    df1=as.data.frame(sort(rowSums(data$flt$counts)));

# checking filtration
  df1=as.data.frame(sort(rowSums(data$flt$counts))); 
  prevdf1 =  apply(data$flt$counts, MARGIN=1, FUN = function(x){sum(x>0)}) # calc prev# for each ASV
...
# filtering tax-object based on counts-filtration  
  data$flt$tax =   data$raw$tax[row.names(data$flt$counts) , ]
n=nrow(df)-nrow(df1)
...
f# pdf(paste0(output$heatmaps, "samples_realALL_flt.pdf"), width=12, height = 8); plot(amp_htmp); dev.off()
```

```{r cleanup heatmap, include=F}
#-----
rm(amp_htmp, label1, alpha)
# amp$sub=NULL
```
...
mockcurve <- amp_rarecurve(data$mock$amp, color_by = "Treatment",  stepsize = 100) + xlim(0,xmx/3) + #facet_by = "Treatment", 
...
sort(colSums(data$mock$amp$abund))
amp_htmp=amp_heatmap(data$mock$amp, tax_aggregate = "Species", facet_by=label1, #group_by = label2, 
...
NegPCR control was not represented by any reads and was dropped from any analysis. 
Heatmap profiles are displayed in absolute abundance, to better represent read richness per sample.
...
#Number of individuals per sample & min individuals
  alpha$reads=colSums(data$flt$counts);  # alpha$reads
  alpha$SpRichness=vegan::specnumber(data$flt$counts, MARGIN = 2);  # alpha$SpRichness #number of species
  alpha$Shannon=vegan::diversity(as.matrix(data$flt$counts), "shannon", MARGIN = 2);   #alpha$Shannon
  alpha$InvSimpson=vegan::diversity(data$flt$counts, "inv", MARGIN = 2);  # alpha$InvSimpson
  alpha$Evenness=alpha$Shannon/log(specnumber(data$flt$counts, MARGIN = 2)); #alpha$Evenness 
  alpha$chao1=apply(data$flt$counts, 2, chao1) #fossil::chao1(data$flt, taxa.row = T)
...
# recreate the ampvis object for experimental data, with flt data
  data$exp$amp=amp_load(cbind(data$flt$counts, data$flt$tax), data$meta)
  data$exp$amp$metadata
```

```{r, metadata and ampvis object}
  data$meta=cbind(data$exp$amp$metadata, round(alpha$temp, 3)) # extending the meta table
  data$meta %>% kable("html") %>% kable_styling("striped", "condensed") %>% 
  scroll_box(width = "100%", height= "500px")
```
...
output$tables=paste0(output$path, "/tables"); dir.create(output$tables)
  ...
  write.table(data$flt$counts, paste0(output$tables, "/sample_flt_ASVtab.txt"), 
              sep="\t", col.names=NA, quote = F, na="")
  write.table(data$meta, paste0(output$tables, "/sample_flt_METAtab.txt"), 
              sep="\t", col.names=NA, quote=F, na="")
  write.table(data$flt$tax, paste0(output$tables, "/sample_flt_TAXtab.txt"), 
              sep="\t", col.names=NA, quote=F, na="")
...
label2="Metadata1"
  ...
  sst=list()
  sst$meta=data$meta
  sst$meta$group1= factor(data$meta[[label1]])
  sst$meta$group2= factor(data$meta[[label2]])#, levels=lvl[[label2]])
...
### --------------
CMTmtx <- vegdist(t(data$flt$counts))
    #gPERMANOVA
cat("============ gPERMANOVA, label1 = ", label1)
q=adonis2(CMTmtx ~ data$meta[[label1]]); 
print(q) 
    #ANOSIM
cat("============ ANOSIM, label1 = ", label1)
q <- anosim(CMTmtx, data$meta[[label1]]); 
print(q)

### --------------------------- ^^^
rm(c,i,q,s, CMTmtx, sub)
```
Note: all samples were "recipients" so donor ~vs~ recipients community composition analysis could not be performed. Community composition showed significant difference based on TreatmentGroup, with this factor being the sole contributor to the differences in the communities (ANOSIM R = 1). 
...
# rm(batch)
save.image(paste0(output$path, "analysisR_v1.RData"))
```
