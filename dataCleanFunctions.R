...
myExport <- function(ampObj, outDIR, biom=F, seqs=F, fastaFile = NULL){
  # create output DIR
  if (!dir.exists(outDIR) ){dir.create(outDIR) }
  
  # export COUNTS+TAXranks
  otutab <- cbind(ampObj$abund, ampObj$tax)
  write.table(otutab,          file.path(paste0(outDIR, "abundtaxTAB.txt")), sep="\t", 
              col.names = NA, quote=F, na="")
  #export COUNTS
  write.table(ampObj$abund,    file.path(paste0(outDIR, "abundanceTAB.txt")), sep="\t", 
              col.names = NA, quote=F, na="") ## we need row names here
  #export TAX
  write.table(ampObj$tax,       file.path(paste0(outDIR, "taxonomyTAB.txt")), sep="\t", 
              col.names = NA, quote=F, na="") ## we need row names here
  #export META
  write.table(ampObj$metadata, file.path(paste0(outDIR, "metadataTAB.txt")), sep="\t", 
  ...
  if (biom == T ){
    ...
    write.table(lineage,      file.path(paste0(outDIR, "lineageTAB.txt")), sep="\t",
                col.names = NA, quote=F, na="")
    # # export COUNTS+LINEAGE (not used in biom)
    lintab <- cbind(ampObj$abund, lineage[1])
    write.table(lintab,       file.path(paste0(outDIR, "abundlinTAB.txt")), sep="\t",
                col.names = NA, quote=F, na="")
    ...
    write_biom(dada2biom(otu = t(ampObj$abund), tax = ampObj$tax), biom_file = paste0(outDIR, "biomJSON.biom") )
  ...
  if (seqs == T) {
    ...
    if (!is.null(fastaFile) && file.exists(fastaFile)) {
      ...
      writeXStringSet(seqsFLT, paste0(outDIR, "sequences.fasta"))
    } else {
