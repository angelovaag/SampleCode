...
rm(x)
print("------> start of TAX Diversity Plots <------")
...
write.table(tables$meta, paste0(path$outdir, "/TAX_AlphaDiv_mapping.txt"), sep="\t", col.names=NA, quote=F, na="")
...
p1=ggplot(sst$melt, aes(group2, value)) +
  geom_boxplot(aes(x=group2, y=as.numeric(as.character(value), stat="identity",
                                               position="dodge") )) + theme_bw() + geom_jitter(aes(col=group1)) +
  facet_wrap(~variable, scales="free") + labs(y="Alpha Diversity Measures", x="TreatmentGroup", col="SampleID") +
...
# head(amp$abund); head(amp$tax); head(amp$metadata)
...
try({
    rrn=min(colSums(amp$abund)); rrx=max(colSums(amp$abund))
    amp2=amp_subset_samples(amp, minreads = rrn , rarefy=rrx) ## captures all samples, no cutoff
    rrc=amp_rarecurve(amp2, color_by = "SampleID", stepsize = round(rrn/500, 0) )
    pdf(paste0(path$outdir, "/TAX_RareficaitonCurve.pdf"), width=12, height=10); plot(rrc); dev.off()
}) # in case of errors the script should not hault
...
#cleanup
rm(amp_htmp, amp2, nmds_bray, p1, pcoa_bray, sst, rrc, rab, rrn, rrx)

#save R image
save.image("TAX_DiversityPlots_v3.RData")
